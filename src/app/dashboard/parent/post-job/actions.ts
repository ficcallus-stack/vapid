...
export async function upsertJobDraft(data: any) {
  ...
  if (existingDraft) {
    await db.update(jobs).set({
      ...
      minRate: hiringType === 'retainer' ? ((retainerBudget || 0) / 40) : (minRate || existingDraft.minRate),
      maxRate: hiringType === 'retainer' ? ((retainerBudget || 0) / 40) : (maxRate || existingDraft.maxRate),
      ...
      retainerBudget: retainerBudget ? retainerBudget * 100 : existingDraft.retainerBudget,
    ...
    await db.insert(jobs).values({
      ...
      minRate: hiringType === 'retainer' ? ((retainerBudget || 0) / 40) : (minRate || 20),
      maxRate: hiringType === 'retainer' ? ((retainerBudget || 0) / 40) : (maxRate || 30),
      status: "open",
      isDraft: true,
      scheduleType: scheduleType || "recurring",
      hiringType: hiringType || "hourly",
      retainerBudget: retainerBudget ? retainerBudget * 100 : 120000,
      schedule: schedule || {},
...
export async function createJob(data: any) {
  ...
  if (draft) {
    await db.update(jobs).set({
      ...
      description,
      budget,
      minRate: hiringType === 'retainer' ? ((retainerBudget || 0) / 40) : minRate,
      maxRate: hiringType === 'retainer' ? ((retainerBudget || 0) / 40) : maxRate,
      location,
      duration,
      startDate: startDate ? new Date(startDate) : null,
      childCount,
      ...
      hiringType,
      retainerBudget: retainerBudget ? retainerBudget * 100 : 0,
      status: "open",
      scheduleType,
      schedule,
      specificDates,
